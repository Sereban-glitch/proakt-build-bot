...
internal/
  ...
  domain/     сущности: объект, акт, позиция, оплата, фото
  ...
  parse/      локальный парсер «наименование кол-во ед. цена» (запасной путь без LLM)
  ai/         шлюз: Transcribe (голос→текст), ExtractPositions (текст→JSON-позиции)
  app/        роутер обновлений, голосовой сценарий, фото, меню, FSM
...
- `internal/parse` — локальный парсер позиций: м², м.п., «штукатурка 45 260», суммы без количества
...
- [ ] v0.3 — Google Sheets: прайс и клиенты в Драйве, `/sync` подтягивает изменения
- [ ] v0.4 — Telegram Mini App: таблицы актов и дашборд прямо в мессенджере
