...
Для демонстрации добавлен отдельный кабинет заказчика `/client.html` с четырьмя
разделами: обзор, ход работ, финансы и документы. Главный пример обезличен и
собран из материалов объекта «Парковый 2»: 12 актов на 549 526 ₴, смета
190 383,50 ₴ и реальная цепочка скрытой подготовки. Оплаты и задолженность в
демо условные и подписаны как демонстрационные.

**Интерфейс v0.7** построен вокруг ежедневного сценария мастера: на рабочем
столе первой стоит кнопка новой сметы, рядом — объекты и прайс; главным
денежным показателем стала сумма «к получению». Нижняя навигация содержит
только три основных раздела, а акты, отчёт, прайс и шаблоны собраны в
компактном меню «Ещё». Формы новой сметы и объекта можно открыть прямо с
главного экрана одним нажатием.
...
3. **Перезапусти бота.** При старте он сам вызовет `setChatMenuButton` —
   в чате появится кнопка «🧰 ПрорАКТ 360» (слева от поля ввода). Открой —
   приложение готово.
...
internal/tg/     +1 метод: SetChatMenuButton (кнопка «🧰 ПрорАКТ 360»)
```
