...
export function applyTheme() {
  ...
  root.setProperty('--bg', t.bg_color || '#0f1822');
  root.setProperty('--text', t.text_color || '#f7f5f0');
  root.setProperty('--hint', t.hint_color || '#83909d');
  root.setProperty('--accent', t.button_color || '#f6b73c');
  root.setProperty('--on-accent', readableOn(t.button_color || '#f6b73c'));
  root.setProperty('--accent-text', readableOn(t.button_color || '#f6b73c'));
  root.setProperty('--brand', light ? '#d98c00' : '#f6b73c');
  root.setProperty('--brand-soft', alpha(light ? '#d98c00' : '#f6b73c', light ? .11 : .15));
  ...
  root.setProperty('--bg-glow-1', alpha(t.button_color || '#f6b73c', light ? .10 : .07));
  root.setProperty('--bg-glow-2', alpha(t.link_color || '#f1c40f', light ? .08 : .05));
  root.setProperty('--accent-soft', alpha(t.button_color || '#f6b73c', light ? .12 : .15));
  ...
  root.setProperty('--shadow-accent', `0 10px 26px ${alpha(t.button_color || '#f6b73c', light ? .22 : .27)}`);
