...
// welcome — приветствие (v0.3.8: короткое, «что это даёт» + кнопка инструкции
// под сообщением). Развёрнутые шаги — в instruction(), чтоб не пугать простыней.
func (b *Bot) welcome() string {
	voice := ""
	if b.ai != nil {
		voice = "• позиции можно ДИКТОВАТЬ голосом 🎤 — руки заняты, диктуй прямо с лестницы\n"
	}
	return fmt.Sprintf(`Привет! Я Прораб — твой цифровой прораб на телефоне 🧰

💪 Что это даёт:
• акт в Excel — сразу из телефона: без тетрадок и вечернего переноса
%s• фото скрытых работ — доказательство заказчику при сдаче
• долги и «упущенная выгода» — всегда на виду

Как всем этим пользоваться — кнопка под этим сообщением 👇`, voice)
}
